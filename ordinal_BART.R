ordinal_BART = function(y, X, L, w0 = 0.5, s0 = 0.3, mcmc_iter = 500, mcmc_bn = 250, alpha = 0.95, beta = 2, prob_move = rep(1/3, 3)) {
  ...
  # ks = unlist(lapply(y, function(yi) seq(max(1, yi-1), K-1)))
  ...
  # alpha0_mean = get_prior_mean_prob(probs)
  alpha0 = alpha0_mean
  ...
  for (b in 1:(mcmc_bn+mcmc_iter)) {
    ...
    # print(w)
    # 
    # var_post = 1 / (colSums(Vl * w) + 1 / s0)
    # mean_post = var_post * (colSums(Vl * w * r) + alpha0_mean / s0)
    ...
    for (l in 1:L) {
      ...
      # print(round(tmp_mu_new, 3))
      # print(round(mean_post, 3))
      # print(round(var_post, 3))
      # print(round(mean_post_new, 3))
      # print(round(var_post_new, 3))
